...
inline void writeReflectedProgramLayout(const char *path,
                                        const ReflectedProgramLayout &layout) {
  std::ofstream stream(path);
  if (!stream)
    throw std::runtime_error(std::string("write reflected program layout: ") +
                             path);
  ...
  for (const auto &group : layout.hitGroups) {
    ...
    stream << "hit " << group.slot << " " << group.groupName << " "
           << encodeStageName(group.closestHit) << " "
           << encodeStageName(group.anyHit) << " "
           << encodeStageName(group.intersection) << "\n";
  }
  for (const auto &group : layout.missGroups) {
    if (group.groupName.empty())
      continue;
    stream << "miss " << group.slot << " " << group.groupName << " "
           << group.miss << "\n";
  }
  for (const auto &group : layout.callableGroups) {
    if (group.groupName.empty())
      continue;
    stream << "callable " << group.slot << " " << group.groupName << " "
           << group.callable << "\n";
  }
}
...
inline ReflectedProgramLayout readReflectedProgramLayout(const char *path) {
  ...
  if (!stream)
    throw std::runtime_error(std::string("read reflected program layout: ") +
                             path);
  std::string magic;
  uint32_t version = 0;
  ...
  if (magic != "structural-ray-tracing-layout" || version != 1)
    throw std::runtime_error(
        "unsupported structural ray-tracing layout manifest");
  ...
  while (stream >> kind) {
    if (kind == "hit") {
      ...
      if (!(stream >> group.slot >> group.groupName >> group.closestHit >>
            group.anyHit >> group.intersection))
      ...
      throw std::runtime_error(
          "invalid structural ray-tracing layout manifest");
    }
  }
  return result;
}
