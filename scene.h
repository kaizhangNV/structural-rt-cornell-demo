...
struct Camera {
  ...
  Float3 forward() const {
    ...
    return normalize(
        {std::sin(yaw) * cosPitch, std::sin(pitch), std::cos(yaw) * cosPitch});
  }
  ...
  void move(float forwardAmount, float rightAmount, float upAmount) {
    ...
    position =
        position + horizontalForward * forwardAmount + right() * rightAmount;
    position.y += upAmount;
  }

  FrameData makeFrame(uint32_t width, uint32_t height, uint32_t stride,
                      bool bgra) const {
...
inline Vertex vertex(float x, float y, float z) { return {{x, y, z}}; }

inline Surface surface(float nx, float ny, float nz, float red, float green,
                       float blue) {
  return {{nx, ny, nz, 0.0f}, {red, green, blue, 1.0f}};
}

inline void addTriangle(SceneData &scene, const Vertex &a, const Vertex &b,
                        const Vertex &c, const Surface &material) {
...
inline void addQuad(SceneData &scene, const Vertex &a, const Vertex &b,
                    const Vertex &c, const Vertex &d, const Surface &material) {
  addTriangle(scene, a, b, c, material);
  addTriangle(scene, a, c, d, material);
}

inline void addBox(SceneData &scene, float minX, float minY, float minZ,
                   float maxX, float maxY, float maxZ,
                   const Surface &material) {
  addQuad(scene, vertex(minX, minY, minZ), vertex(minX, minY, maxZ),
          vertex(minX, maxY, maxZ), vertex(minX, maxY, minZ),
          surface(-1.0f, 0.0f, 0.0f, material.albedo[0], material.albedo[1],
                  material.albedo[2]));
  addQuad(scene, vertex(maxX, minY, maxZ), vertex(maxX, minY, minZ),
          vertex(maxX, maxY, minZ), vertex(maxX, maxY, maxZ),
          surface(1.0f, 0.0f, 0.0f, material.albedo[0], material.albedo[1],
                  material.albedo[2]));
  addQuad(scene, vertex(minX, minY, maxZ), vertex(maxX, minY, maxZ),
          vertex(maxX, maxY, maxZ), vertex(minX, maxY, maxZ),
          surface(0.0f, 0.0f, 1.0f, material.albedo[0], material.albedo[1],
                  material.albedo[2]));
  addQuad(scene, vertex(maxX, minY, minZ), vertex(minX, minY, minZ),
          vertex(minX, maxY, minZ), vertex(maxX, maxY, minZ),
          surface(0.0f, 0.0f, -1.0f, material.albedo[0], material.albedo[1],
                  material.albedo[2]));
  addQuad(scene, vertex(minX, minY, minZ), vertex(maxX, minY, minZ),
          vertex(maxX, minY, maxZ), vertex(minX, minY, maxZ),
          surface(0.0f, -1.0f, 0.0f, material.albedo[0], material.albedo[1],
                  material.albedo[2]));
  addQuad(scene, vertex(minX, maxY, maxZ), vertex(maxX, maxY, maxZ),
...
inline SceneData makeScene() {
  ...
  addQuad(scene, vertex(-1.0f, 0.0f, -1.0f), vertex(1.0f, 0.0f, -1.0f),
          vertex(1.0f, 0.0f, 1.0f), vertex(-1.0f, 0.0f, 1.0f),
          surface(0.0f, 1.0f, 0.0f, 0.76f, 0.73f, 0.66f));
  addQuad(scene, vertex(-1.0f, 2.0f, 1.0f), vertex(1.0f, 2.0f, 1.0f),
          vertex(1.0f, 2.0f, -1.0f), vertex(-1.0f, 2.0f, -1.0f),
          surface(0.0f, -1.0f, 0.0f, 0.76f, 0.73f, 0.66f));
  addQuad(scene, vertex(-1.0f, 0.0f, -1.0f), vertex(-1.0f, 2.0f, -1.0f),
          vertex(1.0f, 2.0f, -1.0f), vertex(1.0f, 0.0f, -1.0f),
