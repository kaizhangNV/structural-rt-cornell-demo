...
SceneResources buildScene(IDevice *device, ICommandQueue *queue,
  ...
  check(device->getAccelerationStructureSizes(bottomBuild, &bottomSizes),
  ...
  check(device->createAccelerationStructure(bottomDesc,
                                            scene.bottomLevel.writeRef()),
        "create bottom-level acceleration structure");

  auto commandEncoder = queue->createCommandEncoder();
  commandEncoder->buildAccelerationStructure(
      bottomBuild, scene.bottomLevel, nullptr, bottomScratch, 0, nullptr);
  check(queue->submit(commandEncoder->finish()),
  ...
  std::memcpy(instance.transform, kIdentityTransform,
  ...
  convertAccelerationStructureInstanceDescs(
      1, instanceType, nativeInstance.data(), instanceStride, &instance,
      sizeof(instance));
  ...
  scene.instanceBuffer =
      device->createBuffer(instanceBufferDesc, nativeInstance.data());
  ...
  check(device->getAccelerationStructureSizes(topBuild, &topSizes),
  ...
  check(device->createAccelerationStructure(topDesc, scene.topLevel.writeRef()),
        "create top-level acceleration structure");

  commandEncoder = queue->createCommandEncoder();
  commandEncoder->buildAccelerationStructure(topBuild, scene.topLevel, nullptr,
                                             topScratch, 0, nullptr);
  check(queue->submit(commandEncoder->finish()),
...
// Finds a reflected stage in the root module or one of its transitively loaded
// shader modules.
ComPtr<slang::IEntryPoint>
findEntryPointInLoadedModules(slang::ISession *session, const std::string &name,
                              SlangStage stage) {
  for (SlangInt moduleIndex = 0; moduleIndex < session->getLoadedModuleCount();
       ++moduleIndex) {
    auto module = session->getLoadedModule(moduleIndex);
    ComPtr<slang::IBlob> diagnostics;
    ComPtr<slang::IEntryPoint> entryPoint;
    const auto result = module->findAndCheckEntryPoint(
        name.c_str(), stage, entryPoint.writeRef(), diagnostics.writeRef());
    if (SLANG_SUCCEEDED(result)) {
      printDiagnostics(diagnostics);
      return entryPoint;
    }
  }
  throw std::runtime_error("find reflected entry point " + name);
}

LoadedProgram loadProgram(IDevice *device) {
  auto session = device->getSlangSession();
  ComPtr<slang::IBlob> diagnostics;
  auto module = session->loadModule("raygen", diagnostics.writeRef());
  printDiagnostics(diagnostics);
  if (!module)
    throw std::runtime_error("load shaders/raygen.slang");
  ...
  const auto addStage = [&](const std::string &name, SlangStage stage) {
    ...
    const auto duplicate =
        std::find_if(entries.begin(), entries.end(), [&](const Entry &entry) {
          return entry.name == name && entry.stage == stage;
        });
  ...
  for (const auto &entry : entries) {
    auto entryPoint =
        findEntryPointInLoadedModules(session, entry.name, entry.stage);
  ...
  auto result = session->createCompositeComponentType(
      components.data(), SlangInt(components.size()), composed.writeRef(),
  ...
  result = device->createShaderProgram(programDesc, program.writeRef(),
                                       diagnostics.writeRef());
  printDiagnostics(diagnostics);
...
createPipeline(IDevice *device, IShaderProgram *program,
  ...
  for (size_t slot = 0; slot < layout.hitGroups.size(); ++slot) {
    ...
    group.anyHitEntryPoint =
        reflected.anyHit.empty() ? nullptr : reflected.anyHit.c_str();
    group.intersectionEntryPoint = reflected.intersection.empty()
                                       ? nullptr
                                       : reflected.intersection.c_str();
  ...
  check(device->createRayTracingPipeline(desc, pipeline.writeRef()),
        "create ray-tracing pipeline");
  return pipeline;
}

ComPtr<IShaderTable> createShaderTable(IDevice *device, IShaderProgram *program,
                                       const ReflectedProgramLayout &layout) {
  ...
  check(device->createShaderTable(desc, table.writeRef()),
        "create shader table");
  return table;
}

void writePpm(const char *path, const uint32_t *pixels, uint32_t width,
              uint32_t height) {
...
uint64_t imageChecksum(const uint32_t *pixels, uint32_t width,
                       uint32_t height) {
...
RenderResources createRenderer(const char *shaderDirectory,
  ...
  check(getRHI()->createDevice(deviceDesc, renderer.device.writeRef()),
  ...
  renderer.pipeline =
      createPipeline(renderer.device, renderer.program, renderer.programLayout);
  renderer.shaderTable = createShaderTable(renderer.device, renderer.program,
                                           renderer.programLayout);
  ...
  renderer.surfaces =
      renderer.device->createBuffer(surfaceDesc, sceneData.surfaces.data());
...
void renderFrame(RenderResources &renderer, IBuffer *output,
                 const cornell::FrameData &frame, ITexture *destination,
                 Size rowPitch) {
  ...
  check(root["scene"].setBinding(Binding(renderer.scene.topLevel)),
        "bind scene");
  check(root["surfaces"].setBinding(Binding(renderer.surfaces)),
        "bind surfaces");
  ...
  if (destination) {
    commandEncoder->copyBufferToTexture(
        destination, 0, 0, {}, output, 0, rowPitch * frame.imageSize[1],
        rowPitch, {frame.imageSize[0], frame.imageSize[1], 1});
  }
  check(renderer.queue->submit(commandEncoder->finish()), "submit ray tracing");
}

int runHeadless(const char *shaderDirectory, const char *outputPath,
                const char *reflectionOutput) {
  ...
  renderFrame(renderer, output, camera.makeFrame(width, height, width, false),
              nullptr, 0);
  check(renderer.queue->waitOnHost(), "wait for ray tracing");

  ComPtr<ISlangBlob> image;
  check(renderer.device->readBuffer(output, 0, outputSize, image.writeRef()),
        "read output buffer");
...
int runInteractive(const char *shaderDirectory, uint32_t maximumFrames,
                   const char *reflectionOutput) {
...
#else
  const auto windowHandle = WindowHandle::fromXlibWindow(window.nativeDisplay(),
                                                         window.nativeWindow());
#endif
  ...
  const bool bgra =
      format == Format::BGRA8Unorm || format == Format::BGRA8UnormSrgb;
  Size rowAlignment = 1;
  check(renderer.device->getTextureRowAlignment(format, &rowAlignment),
        "get row alignment");
  ...
  while (window.poll(input)) {
    ...
    if (!target)
      continue;
    renderFrame(renderer, output,
                camera.makeFrame(configuredWidth, configuredHeight,
                                 uint32_t(rowPitch / 4), bgra),
                target, rowPitch);
...
int main(int argc, char **argv) {
  try {
    ...
    return headless ? runHeadless(shaderDirectory, outputPath, reflectionOutput)
                    : runInteractive(shaderDirectory, maximumFrames,
                                     reflectionOutput);
