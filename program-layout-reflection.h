...
inline std::string getReflectedStageName(slang::RayTracingStageReflection* stage)
{
...
// group slots.
inline ReflectedProgramLayout reflectProgramLayout(
    slang::ProgramLayout* program,
    const char* typeName)
{
    auto reflection = program->findTraceProgramLayout(typeName);
    if (!reflection)
        throw std::runtime_error(std::string("reflect trace program layout: ") + typeName);
    ...
    for (SlangUInt i = 0; i < reflection->getHitGroupCount(); ++i)
    {
        ...
        placeReflectedGroup(result.hitGroups, reflectedGroup->getSlot(), std::move(group));
    }
    for (SlangUInt i = 0; i < reflection->getMissGroupCount(); ++i)
    {
        ...
        placeReflectedGroup(result.missGroups, reflectedGroup->getSlot(), std::move(group));
    }
    for (SlangUInt i = 0; i < reflection->getCallableGroupCount(); ++i)
    {
        ...
        placeReflectedGroup(result.callableGroups, reflectedGroup->getSlot(), std::move(group));
    }
